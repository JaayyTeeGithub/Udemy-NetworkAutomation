...
async def fetch(url):
    async with aiohttp.ClientSession() as session:
        response = await session.get(url) # On this line I'm calling the session.get() coroutine and as any coroutine,
        # I must await it.
...
async def main(urls):
    tasks = [] # each coroutine will create a task that will be spawned asynchronously.
    # The tasks will be saved into a list.
    for url in urls:
